        ...
        <div class="card card-primary card-outline">
          ...
          <div class="card-body table-responsive p-0">
            <table class="table table-hover text-nowrap">
              ...
              <tbody>
                <tr v-for="(s_masuk, index ) in s_masuks" :key="s_masuk.id">
                  <td>{{index+1}}</td>
                  <td>{{s_masuk.no_surat}}</td>
                  <td>{{s_masuk.judul}}</td>
                  <td>{{s_masuk.dari}}</td>
                  <td>
                    <span class="badge badge-secondary">{{s_masuk.tgl_surat | tanggal}}</span>
                  </td>
                  <td>
                  ...
                  <td>
                    <button class="btn btn-sm btn-success" @click="editModal(s_masuk)">
                      <i class="fas fa-edit fa-fw"></i>
                      Edit
                    </button>
                    <button class="btn btn-sm btn-danger" @click="deleteSurat(s_masuk.id)">
            ...
            <div class="modal-body">
              ...
              <div class="form-group">
                <input
                  ...
                  name="dari"
                  class="form-control"
                  :class="{ 'is-invalid': form.errors.has('dari') }"
                  placeholder="Masukan dari"
                />
                <has-error :form="form" field="dari"></has-error>
              </div>
...
export default {
  ...
  methods: {
    ...
    editModal(s_masuk) {
      ...
      this.form.reset();
      $("#addNew").modal("show");
      this.form.fill(s_masuk);
    },
    loadSurat() {
      this.$Progress.start();
      axios.get("api/s_masuk").then(({ data }) => (this.s_masuks = data.data));
