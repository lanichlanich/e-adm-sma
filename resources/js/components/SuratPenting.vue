      ...
      <div class="col-md-12">
        <div class="card card-primary card-outline">
          <div class="card-header">
            <h3 class="card-title">Daftar Surat Penting</h3>
          ...
          <div class="card-body table-responsive p-0">
            <table class="table table-hover text-nowrap">
              ...
              <tbody>
                <tr v-for="(s_penting, index ) in s_pentings" :key="s_penting.id">
                  <td>{{index+1}}</td>
                  <td>{{s_penting.no_surat}}</td>
                  <td>{{s_penting.judul}}</td>
                  <td>{{s_penting.jenis}}</td>
                  <td>
                    <span class="badge badge-secondary">{{s_penting.tgl_surat | tanggal}}</span>
                  </td>
                  <td>
                  ...
                  <td>
                    <button class="btn btn-sm btn-success" @click="editModal(s_penting)">
                      <i class="fas fa-edit fa-fw"></i>
                      Edit
                    </button>
                    <button class="btn btn-sm btn-danger" @click="deleteSurat(s_penting.id)">
...
export default {
  ...
  methods: {
    ...
    editModal(s_penting) {
      ...
      this.form.reset();
      $("#addNew").modal("show");
      this.form.fill(s_penting);
    },
    loadSurat() {
      this.$Progress.start();
      axios
        .get("api/s_penting")
        .then(({ data }) => (this.s_pentings = data.data));
    ...
    updateSurat() {
      this.$Progress.start();
      this.form
        .put("api/s_penting/" + this.form.id)
        .then(() => {
          $("#addNew").modal("hide");
          this.$emit("afterSubmit");
          this.$Progress.finish();
